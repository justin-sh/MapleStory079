    ...
    public static class 召唤永久的怪物 extends CommandExecute {
        ...
        public int execute(final MapleClient c, final String[] splitted) {
            ...
            if (npc != null && !npc.getName().equals("MISSINGNO")) {
                ...
                try {
                    final Connection con = DatabaseConnection.getConnection();
                    try (final PreparedStatement ps = con.prepareStatement("INSERT INTO wz_customlife (dataid, f, hide, fh, cy, rx0, rx1, type, x, y, mid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
                        ps.setInt(1, npcId);
                        ps.setInt(2, 0);
                        ps.setInt(3, 0);
                        ps.setInt(4, fh);
    ...
    public static class RemoveItemOff extends CommandExecute {
        ...
        public int execute(final MapleClient c, final String[] splitted) {
            ...
            try {
                ...
                final String name = splitted[2];
                final PreparedStatement ps = dcon.prepareStatement("select * from characters where name = ?");
                ps.setString(1, name);
                try (final ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        id = rs.getInt("id");
